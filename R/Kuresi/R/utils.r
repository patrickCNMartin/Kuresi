...
load_visium <- function(input_dir, tag = NULL) {
    ...
    if (grepl(pattern  = "spatial", x  = sub_folder)){
        coord <- list.files(path = paste0(input_dir,"/spatial"),
            pattern = "tissue_positions",
            full.names = TRUE)
        if (length(coord) == 0) {
            stop(paste0("No Spatial coordinates found in input directory:",
            input_dir,"/spatial"))
        }
        image <- list.files(path = paste0(input_dir,"/spatial"),
            pattern = "hires_image|lowres_image",
            full.names = TRUE)
        if (length(image) == 0) {
            stop(paste0("No hires_image found in input directory: ",
            input_dir,"/spatial"))
        }
        js <- list.files(path = paste0(input_dir,"/spatial"),
            pattern = ".json",
            full.names = TRUE)
        if (length(js) == 0) {
            stop(paste0("No scaling data (.json) found in input directory:",
            input_dir,"/spatial"))
        }
    } else {
    #-------------------------------------------------------------------------#
    ...
    #-------------------------------------------------------------------------#
        coord <- list.files(path = input_dir,
            pattern = "tissue_positions",
            full.names = TRUE)
        if (length(coord) == 0) {
            stop(paste0("No Spatial coordinates found in input directory:",
        ...
        image <- list.files(path = input_dir,
            pattern = "hires_image|lowres_image",
            full.names = TRUE)
        if (length(image) == 0) {
            stop(paste0("No hires_image found in input directory:",
...
win_lose_genes <- function(){
    win <- toupper(c("Myc", "Mycn", "Cacfd1", "Col17a1", "Yap1", "Taz",
...
cancer_maker_list <- function(type) {
    genes <- switch(type,
                    "pancreas" = c("BRCA1", "BRCA2", "PALB2", "CDKN2A", "ATM",
                                    "TP53","STK11","MLH1","MSH2",
                                    "MSH6","PMS2","EPCAM"),
                    "breast" = c("BRCA1", "BRCA2","CHEK2","PALB2","TM",
                                 "ATM","BARD1","BRIP1","CHEK1",
                                 "CDH1","NF1","PTEN","RAD51C","RAD51D",
                                 "STK11","TP53"),
                    "ovary" = c("ATM","BRCA1","BRCA2","BRIP1","EPCAM",
...
function_call <- function(score_type,
    single = TRUE,
    by_territory = TRUE) {
...
#' @export 
module_score <- function(vesalius_assay,
    seurat_object = NULL,
    gene_list,
    threshold = 0,
    filter = FALSE,
    ...
    if (!is.null(add_name)){
        new_trial <- create_trial_tag(colnames(territories),
             add_name) %>%
            tail(1)
    } else {
        new_trial <- create_trial_tag(colnames(territories),
...
#' @export
compress_score <- function(vesalius_assay,
    trial = "last",
    add_name = NULL,
    rank = TRUE,
    bins = 1) {
...
pseudo_map <- function(query_assay, seed_assay){
    from <- vesalius::get_coordinates(query_assay)$barcodes
    to <- vesalius::get_coordinates(seed_assay)$barcodes
    map <- data.frame("from" = from, "to" = to)
    query_assay@map <- map
    return(query_assay)
}
...
quick_assay_build <- function(seed_assay,
    query_assay,
    integrated,
    seed_trial,
    query_trial,
    ...
    territories <- data.frame(
        ...
        "sample" = c(rep("reference", nrow(seed_ter)),
            rep("matched",nrow(query_ter))),
        "trial" = c(seed_ter[, seed_trial],
            query_ter[, query_trial]))
    colnames(territories) <- gsub("trial",
